...
namespace Kwmkade.UI.Dialog
{
    ...
    public class CommonDialog : IDialog
    {
        ...
        private List<DialogButton.ActionButton> actionButtons;
        ...
        void Start()
        {
            ...
            addValue = 5f;
            enabled = false;
            actionButtons = new List<DialogButton.ActionButton>();
        ...
        public void ShowDialog(string txtTitle, string txtMessage, DialogButton.ActionButton[] acts = null, Action actClosed = null, bool needCloseByTapBG = false)
        {
            // 手前に表示
            ToFront();
            // タッチを受け付ける
        ...
        public override void OnClickButton(int idx)
        {
            DialogButton.ActionButton btn = actionButtons[idx];
            btn.action?.Invoke();
            // どのボタンを押してもダイアログは閉じる
            CloseDialog();
        }
